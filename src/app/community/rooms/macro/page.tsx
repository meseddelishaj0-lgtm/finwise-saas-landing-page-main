...
export default function MacroInsightsRoom() {
  const [posts, setPosts] = useState<Post[]>([]);
  const [newPost, setNewPost] = useState('');

  useEffect(() => {
    const saved = localStorage.getItem('room_macro_insights');
    if (saved) setPosts(JSON.parse(saved));
  }, []);

  useEffect(() => {
    localStorage.setItem('room_macro_insights', JSON.stringify(posts));
  }, [posts]);
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="bg-white/80 p-5 rounded-2xl shadow-sm mb-10">
          <textarea
            ...
            placeholder="Share your macro insights..."
            rows={3}
        ...
        {posts.map((p) => (
          <motion.div
            key={p.id}
            className="bg-white/90 p-5 rounded-xl shadow mb-5 border border-white/10"
