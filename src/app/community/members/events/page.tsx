...
export default function EventsPage() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('member_events');
    if (saved) setPosts(JSON.parse(saved));
  }, []);

  useEffect(() => {
    localStorage.setItem('member_events', JSON.stringify(posts));
  }, [posts]);
  ...
  const addComment = (id: number, text: string) => {
    if (!text.trim()) return;
    const comment: Comment = { id: Date.now(), author: 'Attendee', content: text, createdAt: new Date().toLocaleString() };
  ...
  return (
    <div className="bg-white/80 p-5 rounded-2xl shadow-sm mb-10">
      <textarea
        ...
        placeholder="Share details about a meetup, event, or networking idea..."
        rows={3}
