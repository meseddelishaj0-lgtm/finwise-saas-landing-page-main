...
export default function BusinessPlanningPage() {
  ...
  const fetchModules = async () => {
    try {
      setLoading(true);
      const res = await fetch("/api/business-planning");
  ...
  return (
    <main className="min-h-screen bg-night text-ivory">
      <div className="max-w-4xl mx-auto px-6 md:px-10 py-14 md:py-20">
        <Reveal>
          <CommandLine cmd="BIZ" note="business planning modules" className="mb-4" />
          ...
          <p className="mt-5 max-w-2xl text-lg text-gray-400 leading-relaxed">
            {"Build a robust business plan framework — from vision and model to operations and funding. Guided, step-by-step and tailored for startup founders."}
          </p>
        </Reveal>
        ...
        <div className="mt-12 grid gap-6">
          {loading ? (
            ...
            <p className="text-gray-500">{"No content available currently."}</p>
          ) : (
            modules.map((m, i) => (
            ...
            <div className="flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
              <div className="max-w-2xl">
                ...
                <p className="mt-4 text-gray-400 leading-relaxed">
                  {"Join the WallStreetStocks entrepreneurship community for live workshops, founder stories, and investor-ready strategy insights."}
                </p>
              </div>
