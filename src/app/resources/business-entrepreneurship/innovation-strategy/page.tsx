...
export default function InnovationStrategyPage() {
  ...
  const fetchModules = async () => {
    try {
      setLoading(true);
      const res = await fetch("/api/innovation-strategy");
  ...
  return (
    <main className="min-h-screen bg-night text-ivory">
      <div className="max-w-4xl mx-auto px-6 md:px-10 py-14 md:py-20">
        <Reveal>
          <CommandLine cmd="BIZ" note="innovation strategy modules" className="mb-4" />
          ...
          <p className="mt-5 max-w-2xl text-lg text-gray-400 leading-relaxed">
            {"Drive business transformation through innovation — from ideation to scaling, portfolio management to emerging tech."}
          </p>
        </Reveal>
            ...
            <div className="flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
              <div className="max-w-2xl">
                ...
                <h2 className="mt-3 font-display text-ivory text-3xl md:text-5xl tracking-tight">
                  {"Ready to innovate boldly?"}
                </h2>
                <p className="mt-4 text-gray-400 leading-relaxed">
                  {"Join the WallStreetStocks entrepreneurship community for innovation labs, founder-led workshops and emerging technology insights."}
                </p>
              </div>
