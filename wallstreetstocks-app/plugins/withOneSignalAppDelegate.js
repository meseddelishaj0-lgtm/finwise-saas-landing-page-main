...
//
// It also adds a didRegisterForRemoteNotificationsWithDeviceToken override to
// forward the APNs device token to OneSignal.
...
function withOneSignalAppDelegate(config) {
  return withAppDelegate(config, (config) => {
    ...
    // Notification click handling is done in JS (_layout.tsx) using expo-web-browser.
    // The module-level JS click handler queues cold-start taps and processes them
    // after React mounts, so native handling is not needed.
