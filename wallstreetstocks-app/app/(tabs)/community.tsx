...
// Shows: custom name if set, otherwise username, never email
const getDisplayName = (user: User | null | undefined): string => {
  ...
  if (user.name) {
    ...
    // Only use name if it's different from email prefix (meaning user set a custom name)
  ...
  // Last resort - use a generic name, never show email
  return 'User';
};

// Helper function to get @handle (like StockTwits)
// Shows: username if set, otherwise generate from user id, never email
const getHandle = (user: User | null | undefined): string => {
  if (!user) return 'user';
  if (user.username) return user.username;
  // Generate a handle from user id if no username set
...
export default function CommunityPage() {
  ...
  const { profile: userProfile, loading: userProfileLoading, refreshProfile, getDisplayName: getContextDisplayName } = useUserProfile();
  
  // Get display name from context for current user
  const displayName = getContextDisplayName();
  const username = userProfile?.username || (userProfile?.id ? `user${userProfile.id}` : 'user');
  ...
  const getDisplayUser = (): User | null => {
    // Prioritize the context userProfile which has the custom name/username
    if (userProfile) {
      return {
        id: userProfile.id,
  ...
  const handleOpenProfile = async (user: User) => {
    ...
    console.log('🔵 Opening profile for user:', user.id, 'passed data:', user.name, '@' + user.username);
  ...
  const openCreatePostModal = async () => {
    const userId = getUserId();
    console.log('🟡 Opening create post modal, userId:', userId);
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log('🟡 Modal profile loaded:', profile.name, '@' + profile.username);
        ...
        console.log('🟡 Setting modalUserName to:', name);
        ...
        <View style={styles.modalContainer}>
          ...
          <ScrollView style={styles.modalBody} keyboardShouldPersistTaps="handled">
            ...
            <View style={styles.createPostUser}>
              <Avatar user={modalUserProfile || userProfile || displayUser} size={44} />
            ...
            {selectedPost && (
              <View style={styles.originalPost}>
                <View style={styles.postHeader}>
                  ...
                  >
                    <Text style={styles.username}>
                      {getDisplayName(selectedPost.user)}
                    </Text>
              ...
              <View style={styles.optionsUserHeader}>
                ...
                <View style={styles.optionsUserInfo}>
                  <Text style={styles.optionsUserName}>
                    {getDisplayName(selectedPostForOptions.user)}
                  </Text>
                  <Text style={styles.optionsUserHandle}>
                    @{getHandle(selectedPostForOptions.user)}
                  </Text>
                </View>
              </View>
            )}
