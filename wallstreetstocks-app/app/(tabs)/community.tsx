...
const isAutoGeneratedName = (name: string, email?: string): boolean => {
  ...
  if (/^[a-z0-9]{8,}$/i.test(name)) {
    ...
    // If less than 20% vowels, likely auto-generated (real names typically 30%+)
...
export default function CommunityPage() {
  ...
  // Refresh user profile when tab comes into focus (ensures updated name after editing)
  useFocusEffect(
    useCallback(() => {
      if (authUser?.id) {
        console.log('🔵 Community: Tab focused, refreshing profile...');
        refreshProfile();
      }
